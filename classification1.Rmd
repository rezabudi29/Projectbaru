...
You will need to use `install.packages()` to install any packages that are not already downloaded onto your machine. You then load R packages into your workspace using the `library()` function:
```{r}
library(dplyr)
library(grid)
library(gtools)
...
## Training Objectives
In this workshop, we'll extend our understanding of regression algorithms and see what we've learned in the previous workshop can be extended to solve a different kind of problem: classification problems. More specifically, we'll learn to solve binary and multi-class classification models using machine learning algorithms that are easily understood and in the case of logistic regression, readily interpretable. 
...
- Predicting: Honors degree from academic scores
- Predicting: Flight delay
- Analysis: Credit Risk Modeling with Logistic Regression  

- **Nearest Neighbors Prediction**
- Parameters of k-NN  
- Euclidean Distance  
- Characteristics of k-NN  
- Predicting: Diagnosing breast cancer  
- Predicting: Classifying customers by industry segment
...
**Discussion: Odds of flying on time vs suffering a departure delay**

Now to a less-fun but no less important example: airport delays. If I tell you that the probability of a minor departure delay occuring at a particularly busy airport (Soekarno-Hatta) on a festive holiday season is 0.2, what are the chances (expressed in odds) of you departing on time versus that of a departure delay. Recall the formula: 
...
Now that we've made the range our $p$ that can take on 0 to positive infinity; We still have one task to do - we need to make our probability assume a range smaller than 1, essentially making it take on the range of 0 to 1. To make the probability lesser than 1, we will divide p by a number greater than p. 
...
Many of the key assumptions of linear regression do not hold true in logistic regression. We've learned about the linearity assumption, normality of residuals, and homoskedasticity assumptions in our regression models class - they do not apply in the case of logistic regression.
...
Supposed you work in an education institution and are put in charge to evaluate the likelihood of a student graduating with an honors degree given their academic scores in a reading test, writing test and mathematics test.  
...
Our odds ratio, without the influence of any predictor variables, is 49 out of 200 (49 in honors classes vs 151 not), so that give us a probability of 49/200, p = 0.245. Our odds ratio is therefore 0.245/(1-0.245) = 0.3245033
...
```
Before we attempt to interpret the parameters estimated from our model above, let's examine the odds ratio of a female being in an honors class as we did before:
...
- The ratio of the odds for female vs ratio of the odds for male = .42/.23 = 1.809, which is to say that the odds for female being in an honors class are about 80.9% more than that of their male counterpart  
...
Let's now relate the odds ratio to the ouput from the logistic regression model with our `female` predictor variable. 
```{r}
...
summary(glm(DepDel15 ~ Month + DayofMonth + DayofWeek, data=flights.s, family="binomial")) #month negative yang artinya bulan semakin bertambah, maka probability dari delay bulan desember dibandingkan bulan may, artinya bahwa bulan may lebih sering delay. semakin bertambah bulannya, maka semakin berkurang delay nya
```
...
Interesting weekend read: Another interesting project that models customer retention using historical data from a database (more than 500,000 clients) of a big mutual fund investment company and logistic regression (Eiben, Euverman, Kowalczyk, Slisser[^2]), which highlight the benefits of an interpretative model like the one we obtain with logistic regression.
...
Sensitivity and specificity are metrics commonly used to measure the performance of a binary classiciation.  
...
Well, one thing we can do is to set the treshold to be more sensitive to "positive cases": Let's see what happen if we were to predict a "default" when the probability exceed 0.4 (20% more sensitive than our previous classifier): 
...
```{r}
plot(performance(pred.rocrtest, "tpr", "fpr"),colorize=TRUE, print.cutoffs.at=seq(0,1,by=0.1),text.adj=c(-0,2,1,7)) #plot ini meunjukkan cara untuk memilih threshold
```
...
Null deviance indicates how well the response variable is predicted by a model that includes only the intercept (grand mean). The residual deviance shows how well the response variable is prediced by the model when all predictors are included.
...
## Understanding Parameters of Nearest Neighbors
...
While these distance functions exist, Euclidean distance is far more often seen in industrial applications and is therefore the focus of this chapter. As a side note, Minkowsiki distance is typically used with _r_ being 1 or 2, where the former is equivalent to the Manhatten distance while the latter is the Euclidean distance.  
...
If we use a very large _k_, say, a _k_ value as large as the total number of observations in the training data, this would lead to the model always predicting the majority class (ZeroR classifier), which we've learned about in the previous chapter.
...
Notice we could also have use `diff(range(x))` instead of `max(x) - min(x)`. `range(x)` returns a vector of two, the min and max, so diff() will calculate the difference between that two values. Let's test our normalize functions on a few vectors:
```{r}
...
Observe that 61 of 100 values are benign and the model correctly identifies them as such (**true negative**), and on the opposite end there are 37 **true positives**. 

The classifier also misclassified two values as benign when the clinical results were malignant -- this is formally referred to as **false negative**. FN could be extremely costly as it may lead to a patient believing that she is cancer-free and allow the cancerous cell to spread.  
...
**Graded Assignment: Understanding Logistic Regression**  
...
Study the logistic regression model we've developed earlier, `honors.logit4`, and answer the quizzes to the best of your ability:  

Question 1: Which variables positively correlates to the increase of odds of graduating with an honors degree? Select **All** that apply. 
- Math (Higher scores in mathematics positively contributes to the probability of graduating with honors)  
- Read (Higher scores in reading positively contributes to the probability of graduating with honors)  
- Female (Being female positively contributes to the probability of graduating with honors)

```{r}
honors.logitGrade <- glm(hon ~ math + female + read, data=honors, family="binomial") 
summary(honors.logitGrade)
```
...
Question 2: While holding *math* and *reading* at a fixed value, the odds of getting into an honors class for females (female = 1) over the odds of getting into an honors class for males is ..?
- 0.97995, or 3% lower
- 2.66432, or 166% higher  
- 1.98015, or 98% higher
```{r}
...
Question 3: 
Complete the following code, using 15 nearest neighbors and the `knn()` library, create your prediction:
```{r}
...
What is the number of misclassification from `horeca_pred`?
Answer:  ____ 
...
1. The document demonstrates the student's ability in data preparation  (eg. one or two exploratory data analysis steps)
2. The document demonstrates the student's understanding of an unbiased estimate of the model's accuracy (eg. train and test sets or cross-validation sets)  
...
Option 2: Customer segment prediction  
Applying what you’ve learned, present a simple R Markdown document in which you demonstrate the use of knn on the `wholesale.csv` dataset. Compare the k-NN to the logistic regression model and answer the following questions throughout the document:  
